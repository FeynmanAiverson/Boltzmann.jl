...
function run_mnist()
    ...
    fit(m, TrainSet;n_iter=Epochs,lr=0.05,validation=ValidSet,n_gibbs=Gibbs)
    ...
    fit(mwdQuad, TrainSet;n_iter=Epochs,weight_decay="l2",decay_magnitude=0.05,lr=0.05,validation=ValidSet,n_gibbs=Gibbs)
    ...
    fit(mwdLin, TrainSet;n_iter=Epochs,weight_decay="l1",decay_magnitude=0.05,lr=0.05,validation=ValidSet,n_gibbs=Gibbs)
